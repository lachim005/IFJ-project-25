...
\begin{document}
    ...
    \section{Syntaktický analýza}
        Syntaktická analýza je implementována metodou rekurzivního sestupu podle LL gramatiky (\ref{app:llgram}) a LL tabulky (\ref{llTab}).
        ...
        Výsledkem syntaktické analýzy je ASS reprezentující celý program a globální tabulka symbolů obsahující všechny definované symboly.
    ...
    \subsection{Zpracování výrazů}
        ...
        Přesněji je využit precedenční syntaktický analyzátor, který výraz zpracovává pomocí práce s precedencní tabulkou (\ref{precTab}).
        ...
        Pokud analyzátor narazí na ')', která není součástí žádného jeho pravidla, tak tuto závorku vrátí voláním funkce \texttt{lexer\_unget\_token()}.

        Po skončení zpracování výrazu je následně volajícímu (\texttt{parse()}) vrácen otcovský uzel celého tohoto výrazu. 
